...
const UserDashboard = () => {
  ...
  const fetchSkill = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:8713/api/skills/${skillId}`);
  ...
  const updateFormData = async (id) => {
    ...
    try {
      const response = await axios.patch(`http://localhost:8713/api/formdata/${id}`, formDataObj, {
        headers: {
  ...
  const updateSkill = async (e) => {
    ...
    try {
      const response = await axios.patch(`http://localhost:8713/api/skills/${skillId}/details`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
    ...
    <div className="max-w-4xl mx-auto p-9 mt-[7%] bg-white rounded-lg shadow-md">
      ...
      {skillData && (
        <div className="skill-info mt-6">
          ...
          <img
            src={`http://localhost:8713${profilePicture}` || 'default_profile_picture_url.jpg'}
            alt="Profile"
                  ...
                  <div key={key} className="mb-2">
                    ...
                    {key.includes('Chapter') || key.includes('roadmapIntroduction') ? (
                      <>
                        {form[key] && (
                          <a
                            href={`http://localhost:8713/pdfUploads/${form[key]}`}
                            target="_blank"
                      ...
                      <>
                        {form.image && (
                          <img
                            src={`http://localhost:8713/imageUploads/${form.image}`}
                            alt="Formdataimage"
