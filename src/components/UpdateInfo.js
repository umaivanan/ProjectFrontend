...
const UpdateInfo = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const formResponse = await axios.get(`http://localhost:8714/api/formdata/${id}`);
        setData(formResponse.data); // Update form data

        const skillResponse = await axios.get('http://localhost:8714/api/skills');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8714/api/formdata/${id}`, formData, {
        headers: {
      ...
      if (userSkill) {
        ...
        await axios.patch(`http://localhost:8714/api/skills/${userSkill._id}`, { formDataId });
      }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            {userSkill?.profilePicture && (
              <img
                src={`http://localhost:8714${userSkill.profilePicture}`} 
                alt="Profile"
